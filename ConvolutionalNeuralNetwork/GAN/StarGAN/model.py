...
class Generator(tf.keras.Model):
    ...
    def call(self, inputs, training=True):
        ...
        domains = tf.cast(domains, tf.float16)
    ...
    def summary(self):
        ...
        model = tf.keras.Model(inputs=[x1, x2], outputs=self.call([x1, x2]))
        return model.summary()


class Discriminator(tf.keras.Model):
    def __init__(self, img_shape, nd, ndf=64, n_layers=3, use_bias=False):
