...
class StudentModel(torch.nn.Module):
    def __init__(self):
        ...
        self.feature_exctractor = torch.nn.Sequential(
            ...
            torch.nn.Tanh(),
            torch.nn.AvgPool2d(kernel_size=2),
            torch.nn.Conv2d(in_channels=8, out_channels=60,
                            kernel_size=5, stride=1),
            torch.nn.Tanh()
        )
...
def distill_loss(pred, target, temp=5):
    softmax = torch.nn.Softmax(dim=1)
    mse = torch.nn.MSELoss()
    soft_pred = softmax(pred/temp)
    soft_target = softmax(target/temp)

    return mse(soft_pred, soft_target)


def train_distill(teacher, student, train_loader, test_loader, epochs=10):
    pass
...
if __name__ == '__main__':
    ...
    batch_size = 64
    torchinfo.summary(student_model, input_size=(batch_size, 1, 28, 28))

    teacher_model = TeacherModel().to(device)
    batch_size = 64
    torchinfo.summary(teacher_model, input_size=(batch_size, 1, 28, 28))

    print('Training teacher model:\n')
    train(teacher_model, train_data_loader, test_data_loader)

    print('Training student model:\n')
    train(student_model, train_data_loader, test_data_loader)
